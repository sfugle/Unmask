...
void UUMAnimDataController::CloseBracket(bool bShouldTransact /*= true*/)
{
	...
	if (BracketDepth == 0)
	{
		UE_LOG(LogScript, Error, TEXT("Attempt to close bracket that was not previously opened"));
		return;
	}
...
void UUMAnimDataController::SetNumberOfFrames(FFrameNumber Length, bool bShouldTransact)
{
	ValidateModel();
...
void UUMAnimDataController::ResizeNumberOfFrames(FFrameNumber NewLength, FFrameNumber T0, FFrameNumber T1, bool bShouldTransact)
{
	ValidateModel();
		...
		if (NewLength != Model->GetNumberOfFrames())
		{
			...
			if (PlayRange.Contains(T0))
			{
				...
				if (T0 < T1)
				{
					...
					// ReSharper disable 3 CppDeprecatedEntity
				...
				{
					UE_LOG(LogScript, Error, TEXT("InvalidEndTimeError: Invalid T1, smaller that T0 value: T0 %d, T1 %d"), T0.Value, T1.Value);
				}
			}
			else
			{
				UE_LOG(LogScript, Error, TEXT("InvalidStartTimeError: Invalid T0, not within existing play range: T0 %d, Play Length %f"), T0.Value, Model->GetPlayLength());
			}
		}
		else if (Model->bPopulated)
		{
			UE_LOG(LogScript, Warning, TEXT("SamePlayLengthWarning: New play length is same as existing one: %d frames"), NewLength.Value);
		}
	}
	else
	{
		UE_LOG(LogScript, Error, TEXT("InvalidPlayLengthError: Invalid play length value provided: %d frames"), NewLength.Value);
	}
}

void UUMAnimDataController::ResizeInFrames(FFrameNumber NewLength, FFrameNumber T0, FFrameNumber T1, bool bShouldTransact)
{
	ValidateModel();
	...
	if (NewLength >= 0)
	{
		if (NewLength != Model->GetNumberOfFrames())
		{
            ...
            {
                UE_LOG(LogScript, Error, TEXT("InvalidStartTimeError: Invalid T0, not within existing play range: T0 %d, Play Length %d"), T0.Value, CurrentNumberOFrames);
            }			
        }
		else if (Model->bPopulated)
        {
            UE_LOG(LogScript, Warning ,TEXT("SameGetPlayLengthWarning:    New play length is same as existing one: %d frames"), CurrentNumberOFrames);
        }
    }
    else
    {
        UE_LOG(LogScript, Error, TEXT("InvalidGetPlayLengthError: Invalid play length value provided: %d frames"), CurrentNumberOFrames);
    }
}

void UUMAnimDataController::SetPlayLength(float Length, bool bShouldTransact /*= true*/)
{
	SetNumberOfFrames(ConvertSecondsToFrameNumber(Length), bShouldTransact);
}

void UUMAnimDataController::Resize(float Length, float T0, float T1, bool bShouldTransact /*= true*/)
{
	ValidateModel();
...
void UUMAnimDataController::SetFrameRate(FFrameRate FrameRate, bool bShouldTransact /*= true*/)
{
	ValidateModel();
	...
	if ( FrameRate.IsValid() && !FMath::IsNearlyZero(FrameRateInterval) && FrameRateInterval > 0.0)
	{
        ...
        {
        	UE_LOG(LogScript, Error, TEXT("NonCompatibleFrameRateError: Incompatible frame rate provided: %s not a multiple or fact or %f"), ToCStr(FrameRate.ToPrettyText().ToString()), CurrentFrameRate.AsDecimal());
        }
	}
	else
	{
		UE_LOG(LogScript, Error, TEXT("InvalidFrameRateError: Invalid frame rate provided: %s"), ToCStr(FrameRate.ToPrettyText().ToString()));
	}
}

bool UUMAnimDataController::RemoveBoneTracksMissingFromSkeleton(const USkeleton* Skeleton, bool bShouldTransact /*= true*/)
{
	...
	{
		UE_LOG(LogScript, Error, TEXT("InvalidSkeletonError: Invalid USkeleton supplied"));
	}

	return false;
}

void UUMAnimDataController::UpdateAttributesFromSkeleton(const USkeleton* Skeleton, bool bShouldTransact)
{
	ValidateModel();
	...
	{
		UE_LOG(LogScript, Error, TEXT("Invalid USkeleton supplied"));
	}
}

void UUMAnimDataController::ResetModel(bool bShouldTransact /*= true*/)
{
	ValidateModel();
...
bool UUMAnimDataController::AddCurve(const FAnimationCurveIdentifier& CurveId, int32 CurveFlags /*= EAnimAssetCurveFlags::AACF_Editable*/, bool bShouldTransact /*= true*/)
{
	ValidateModel();
		...
		if (IsSupportedCurveType(CurveId.CurveType))
		{
			if (!Model->FindCurve(CurveId))
			{
				...
				default:
					...
					UE_LOG(LogScript, Warning, TEXT("Invalid curve type provided: %s (%d)"), ToCStr(CurveTypeAsString), static_cast<int32>(CurveId.CurveType));
				}
			...
			{
				const FString CurveTypeAsString = GetCurveTypeValueName(CurveId.CurveType);
				UE_LOG(LogScript, Warning, TEXT("Curve with name %s and type %s (%d) already exists"), ToCStr(CurveId.CurveName.ToString()), ToCStr(CurveTypeAsString), static_cast<int32>(CurveId.CurveType));
			}			
		}
		else 
		{
	...
	{
		const FString CurveTypeAsString = GetCurveTypeValueName(CurveId.CurveType);
		UE_LOG(LogScript, Warning, TEXT("Invalid curve identifier provided: name: %s, type: %s"), ToCStr(CurveId.CurveName.ToString()), ToCStr(CurveTypeAsString));
	}

	return false;
}

bool UUMAnimDataController::DuplicateCurve(const FAnimationCurveIdentifier& CopyCurveId, const FAnimationCurveIdentifier& NewCurveId, bool bShouldTransact /*= true*/)
{
	ValidateModel();
				...
				if (Model->FindCurve(CopyCurveId))
				{
					if (!Model->FindCurve(NewCurveId))
					{
						...
						default:
							...
							UE_LOG(LogScript, Warning, TEXT("Invalid curve type provided: %s (%d)"), ToCStr(CurveTypeAsString), static_cast<int32>(SupportedCurveType));
						}
					...
					else
					{
						...
						UE_LOG(LogScript, Warning, TEXT("Curve with name %s and type %s (%d) already exists"), ToCStr(NewCurveId.CurveName.ToString()), ToCStr(CurveTypeAsString), static_cast<int32>(NewCurveId.CurveType));
					}
				}
				else
				{
					const FString CurveTypeAsString = GetCurveTypeValueName(CopyCurveId.CurveType);
					UE_LOG(LogScript, Warning, TEXT("Could not find curve with name %s and type %s (%d) for duplication"), ToCStr(NewCurveId.CurveName.ToString()), ToCStr(CurveTypeAsString), static_cast<int32>(NewCurveId.CurveType));
				}
			}
		}
		else
	...
	{
		UE_LOG(LogScript, Warning, TEXT("Invalid curve identifier provided for duplicate: copy: %s, new: %s"), ToCStr(CopyCurveId.CurveName.ToString()), ToCStr(NewCurveId.CurveName.ToString()));
	}
...
bool UUMAnimDataController::RemoveCurve(const FAnimationCurveIdentifier& CurveId, bool bShouldTransact /*= true*/)
{
	ValidateModel();
	...
	if (CurveId.CurveName != NAME_None)
	{
		if (IsSupportedCurveType(CurveId.CurveType))
		{
			...
			{
				const FString CurveTypeAsString = GetCurveTypeValueName(SupportedCurveType);
				UE_LOG(LogScript, Warning, TEXT("Unable to find curve: %s of type %s"), ToCStr(CurveId.CurveName.ToString()), ToCStr(CurveTypeAsString));
			}
		}
		else
		{
...
void UUMAnimDataController::RemoveAllCurvesOfType(ERawCurveTrackTypes SupportedCurveType /*= ERawCurveTrackTypes::RCT_Float*/, bool bShouldTransact /*= true*/)
{
	ValidateModel();
...
bool UUMAnimDataController::SetCurveFlag(const FAnimationCurveIdentifier& CurveId, EAnimAssetCurveFlags Flag, bool bState /*= true*/, bool bShouldTransact /*= true*/)
{
	ValidateModel();
...
bool UUMAnimDataController::SetCurveFlags(const FAnimationCurveIdentifier& CurveId, int32 Flags, bool bShouldTransact /*= true*/)
{
	ValidateModel();
...
bool UUMAnimDataController::SetTransformCurveKeys(const FAnimationCurveIdentifier& CurveId, const TArray<FTransform>& TransformValues, const TArray<float>& TimeKeys, bool bShouldTransact /*= true*/)
{
	...
	{
		// time value mismatch
		UE_LOG(LogScript, Warning, TEXT("Number of times and key entries do not match: number of time values %d, number of key values %d"), TimeKeys.Num(), TransformValues.Num());
	}
...
bool UUMAnimDataController::SetTransformCurveKey(const FAnimationCurveIdentifier& CurveId, float Time, const FTransform& Value, bool bShouldTransact /*= true*/)
{
	ValidateModel();
...
bool UUMAnimDataController::RemoveTransformCurveKey(const FAnimationCurveIdentifier& CurveId, float Time, bool bShouldTransact /*= true*/)
{
	ValidateModel();
...
bool UUMAnimDataController::RenameCurve(const FAnimationCurveIdentifier& CurveToRenameId, const FAnimationCurveIdentifier& NewCurveId, bool bShouldTransact /*= true*/)
{
	ValidateModel();
		...
		if (CurveToRenameId != NewCurveId)
		{
			if (CurveToRenameId.CurveType == NewCurveId.CurveType)
			{
				...
				else
				{
					...
					UE_LOG(LogScript, Warning, TEXT("Unable to find curve: %s of type %s"), ToCStr(CurveToRenameId.CurveName.ToString()), ToCStr(CurveTypeAsString));
				}
			}
			else
			{
				...
				UE_LOG(LogScript, Warning, TEXT("Different curve types provided between current and new curve names: %s (%s) and %s (%s)"), ToCStr(CurveToRenameId.CurveName.ToString()), ToCStr(CurrentCurveTypeAsString),
					ToCStr(NewCurveId.CurveName.ToString()), ToCStr(NewCurveTypeAsString));
			}
		}
		else
		{
			UE_LOG(LogScript, Warning, TEXT("Provided curve names are the same: %s"), ToCStr(CurveToRenameId.CurveName.ToString()));
		}
		
	}
	else
	{
		UE_LOG(LogScript, Warning, TEXT("Invalid new curve identifier provided: %s"), ToCStr(NewCurveId.CurveName.ToString()));
	}

	return false;
}

bool UUMAnimDataController::SetCurveColor(const FAnimationCurveIdentifier& CurveId, FLinearColor Color, bool bShouldTransact)
{
	ValidateModel();
	...
	if (CurveId.IsValid())
	{
		...
		{
			UE_LOG(LogScript, Warning, TEXT("Changing curve color is currently only supported for float curves"));
		}
	}
	else
	{
		UE_LOG(LogScript, Warning, TEXT("Invalid Curve Identifier : %s"), ToCStr(CurveId.CurveName.ToString()));
	}	

	return false;
}

bool UUMAnimDataController::SetCurveComment(const FAnimationCurveIdentifier& CurveId, const FString& Comment, bool bShouldTransact)
{
	ValidateModel();
	...
	if (CurveId.IsValid())
	{
		...
		{
			UE_LOG(LogScript, Warning, TEXT("Changing curve comment is currently only supported for float curves"));
		}
	}
	else
	{
...
bool UUMAnimDataController::ScaleCurve(const FAnimationCurveIdentifier& CurveId, float Origin, float Factor, bool bShouldTransact /*= true*/)
{
	ValidateModel();
	...
	if (SupportedCurveType == ERawCurveTrackTypes::RCT_Float)
	{
		...
		{
			UE_LOG(LogScript, Warning, TEXT("Unable to find float curve: %s"), ToCStr(CurveId.CurveName.ToString()));
		}
	}
	else
	{
		UE_LOG(LogScript, Warning, TEXT("Scaling curves is currently only supported for float curves"));
	}

	return false;
}

bool UUMAnimDataController::SetCurveKey(const FAnimationCurveIdentifier& CurveId, const FRichCurveKey& Key, bool bShouldTransact)
{
	ValidateModel();
...
bool UUMAnimDataController::RemoveCurveKey(const FAnimationCurveIdentifier& CurveId, float Time, bool bShouldTransact)
{
	ValidateModel();
	...
	if (FRichCurve* RichCurve = Model->GetMutableRichCurve(CurveId))
	{
		...
		{
			UE_LOG(LogScript, Error, TEXT("RichCurveKeyNotFoundError: Unable to find rich curve key: curve name %s, time %f"),
				ToCStr(CurveId.CurveName.ToString()), Time);
		}
	}
...
bool UUMAnimDataController::SetCurveKeys(const FAnimationCurveIdentifier& CurveId, const TArray<FRichCurveKey>& CurveKeys, bool bShouldTransact)
{
	ValidateModel();
...
bool UUMAnimDataController::SetCurveAttributes(const FAnimationCurveIdentifier& CurveId, const FCurveAttributes& Attributes, bool bShouldTransact)
{
	ValidateModel();
...
void UUMAnimDataController::NotifyPopulated()
{
	ValidateModel();
...
void UUMAnimDataController::NotifyBracketOpen()
{
	ValidateModel();
	Model->GetNotifier().Notify(EAnimDataModelNotifyType::BracketOpened);
}

void UUMAnimDataController::NotifyBracketClosed()
{
	ValidateModel();
	Model->GetNotifier().Notify(EAnimDataModelNotifyType::BracketClosed);
}

void UUMAnimDataController::ResizePlayLength(float Length, float T0, float T1, bool bShouldTransact)
{
...
bool UUMAnimDataController::AddBoneCurve(FName BoneName, bool bShouldTransact /*= true*/)
{
	if (!ModelInterface->GetAnimationSequence())
	{
		return false;
...
int32 UUMAnimDataController::InsertBoneTrack(FName BoneName, int32 DesiredIndex, bool bShouldTransact /*= true*/)
{
	if (!ModelInterface->GetAnimationSequence())
	{
		return INDEX_NONE;
	...
	if (TrackIndex == INDEX_NONE)
	{
		...
		{
			UE_LOG(LogScript, Warning, TEXT("Cannot add track with name %s. An animation sequence cannot contain more than 65535 tracks"), ToCStr(BoneName.ToString()));
			return INDEX_NONE;
		}
		else
		{
			...
			if (const UAnimSequence* AnimationSequence = Model->GetAnimationSequence())
			{
				if (const USkeleton* Skeleton = AnimationSequence->GetSkeleton())
				{
					...
					{
						UE_LOG(LogScript, Error, TEXT("UnableToFindBoneIndexWarning: Unable to retrieve bone index for track: %s"), ToCStr(BoneName.ToString()));
					}
				}
				else
				{
					UE_LOG(LogScript, Error, TEXT("Unable to retrieve Skeleton for outer Animation Sequence"));
				}
			}
			else
			{
				UE_LOG(LogScript, Error, TEXT("Unable to retrieve outer Animation Sequence"));
			}
	...
	{
		UE_LOG(LogScript, Warning, TEXT("Track with name %s already exists"), ToCStr(BoneName.ToString()));
	}
	
	return TrackIndex;
}

bool UUMAnimDataController::RemoveBoneTrack(FName BoneName, bool bShouldTransact /*= true*/)
{
	if (!ModelInterface->GetAnimationSequence())
	{
		return false;
	...
	{
		UE_LOG(LogScript, Warning, TEXT("Could not find track with name %s"), ToCStr(BoneName.ToString()));
	}

	return false;
}

void UUMAnimDataController::RemoveAllBoneTracks(bool bShouldTransact /*= true*/)
{
	if (!ModelInterface->GetAnimationSequence())
	{
		return;
...
bool UUMAnimDataController::SetBoneTrackKeys(FName BoneName, const TArray<FVector>& PositionalKeys, const TArray<FQuat>& RotationalKeys, const TArray<FVector>& ScalingKeys, bool bShouldTransact /*= true*/)
{
	...
	if (MaxNumKeys > 0)
	{
		...
		{
			UE_LOG(LogScript, Error, TEXT("InvalidTrackKeyDataError: Invalid track key data, expected uniform data: number of positional keys %d, number of rotational keys %d, number of scaling keys %d"), PositionalKeys.Num(), RotationalKeys.Num(), ScalingKeys.Num());
		}
	}
	else
	{
		UE_LOG(LogScript, Error, TEXT("MissingTrackKeyDataError: Missing track key data, expected uniform data: number of positional keys %d, number of rotational keys %d, number of scaling keys %d"), PositionalKeys.Num(), RotationalKeys.Num(), ScalingKeys.Num());
	}

	return false;
}

bool UUMAnimDataController::SetBoneTrackKeys(FName BoneName, const TArray<FVector3f>& PositionalKeys, const TArray<FQuat4f>& RotationalKeys, const TArray<FVector3f>& ScalingKeys, bool bShouldTransact /*= true*/)
{
...
bool UUMAnimDataController::UpdateBoneTrackKeys(FName BoneName, const FInt32Range& KeyRangeToSet, const TArray<FVector3f>& PositionalKeys, const TArray<FQuat4f>& RotationalKeys, const TArray<FVector3f>& ScalingKeys, bool bShouldTransact)
{	
	if (!CheckOuterClass(UAnimSequence::StaticClass()))
	{
		return false;
		...
		if (bValidPosKeys && bValidRotKeys && bValidScaleKeys)
		{
			...
			if (NumKeysToSet == MaxNumKeys)
			{
				if (FBoneAnimationTrack* TrackPtr = Model->FindMutableBoneTrackByName(BoneName))
				{
					...
					{
						UE_LOG(LogScript, Warning, TEXT("Range of to-be-set bone track (with name %s) keys does not overlap existing range"), ToCStr(BoneName.ToString()));
					}
				}
				else
				{
			...
			{
				UE_LOG(LogScript, Error, TEXT("InvalidKeyIndexRangeError: Invalid key index range bound [%d, %d], expected to equal the size of the positional, rotational and scaling keys %d"),
								 RangeMin,
								 RangeMax,
								 MaxNumKeys);
			}	
...
bool UUMAnimDataController::UpdateBoneTrackKeys(FName BoneName, const FInt32Range& KeyRangeToSet, const TArray<FVector>& PositionalKeys, const TArray<FQuat>& RotationalKeys, const TArray<FVector>& ScalingKeys, bool bShouldTransact)
{
	if (!CheckOuterClass(UAnimSequence::StaticClass()))
	{
		return false;
		...
		if (bValidPosKeys && bValidRotKeys && bValidScaleKeys)
		{
			...
			else
			{
				UE_LOG(LogScript, Error,
				       TEXT("InvalidKeyIndexRangeError: Invalid key index range bound [%d, %d], %s %d"),
				       KeyRangeToSet.GetLowerBoundValue(),
				       KeyRangeToSet.GetUpperBoundValue(),
				       ToCStr("expected to equal the size of the positional, rotational and scaling keys"),
						MaxNumKeys
				);
			}	
		}
...
bool UUMAnimDataController::AddAttribute(const FAnimationAttributeIdentifier& AttributeIdentifier, bool bShouldTransact /*= true*/)
{
	if (AttributeIdentifier.IsValid())
	{
		...
		{
			UE_LOG(LogScript, Error, TEXT("AttributeAlreadyExists: Attribute identifier provided already exists: %s %s (%d) %s"),
				ToCStr(AttributeIdentifier.GetName().ToString()), ToCStr(AttributeIdentifier.GetBoneName().ToString()),
				AttributeIdentifier.GetBoneIndex(), ToCStr(AttributeIdentifier.GetType()->GetFName().ToString())
				);
		}
	}
	else
	{
		UE_LOG(LogScript, Error, TEXT("Invalid attribute identifier provided"));
	}

	return false;
}

bool UUMAnimDataController::RemoveAttribute(const FAnimationAttributeIdentifier& AttributeIdentifier, bool bShouldTransact /*= true*/)
{
	if (AttributeIdentifier.IsValid())
	{
		...
		{
			UE_LOG(LogScript, Error, TEXT("AttributeNotFound: Attribute identifier provided was not found: %s %s (%d) %s"),
				ToCStr(AttributeIdentifier.GetName().ToString()), ToCStr(AttributeIdentifier.GetBoneName().ToString()),
				AttributeIdentifier.GetBoneIndex(), ToCStr(AttributeIdentifier.GetType()->GetFName().ToString()));
		}
	}
...
int32 UUMAnimDataController::RemoveAllAttributesForBone(const FName& BoneName, bool bShouldTransact)
{
...
int32 UUMAnimDataController::RemoveAllAttributes(bool bShouldTransact)
{
	int32 NumRemovedAttributes = 0;
...
bool UUMAnimDataController::SetAttributeKey_Internal(const FAnimationAttributeIdentifier& AttributeIdentifier, float Time, const void* KeyValue, const UScriptStruct* TypeStruct, bool bShouldTransact /*= true*/)
{
	if (AttributeIdentifier.IsValid())
	{
		if (KeyValue)
		{
			...
			if (AttributePtr)
			{
				...
				{
					UE_LOG(LogScript, Error, TEXT("AttributeTypeDoesNotMatchKeyType: Key type does not match attribute: %s %s"),
						ToCStr(AttributePtr->Identifier.GetType()->GetFName().ToString()), ToCStr(TypeStruct->GetFName().ToString()));
				}
			}
			else
			{
		...
		{
			UE_LOG(LogScript, Error, TEXT("Invalid attribute key value provided"));
		}
	}
	else
	{
...
bool UUMAnimDataController::SetAttributeKeys_Internal(const FAnimationAttributeIdentifier& AttributeIdentifier, TArrayView<const float> Times, TArrayView<const void*> KeyValues, const UScriptStruct* TypeStruct, bool bShouldTransact)
{
	if (AttributeIdentifier.IsValid())
	{
		...
		{
			UE_LOG(LogScript, Error, TEXT("AttributeKeysMismatch: Non matching number of key time/values: time entries %d key entries %d"),
				Times.Num(), KeyValues.Num()
			);
		}
	}
	else
...
bool UUMAnimDataController::RemoveAttributeKey(const FAnimationAttributeIdentifier& AttributeIdentifier, float Time, bool bShouldTransact /*= true*/)
{
	if (AttributeIdentifier.IsValid())
	{
		FAnimatedBoneAttribute* AttributePtr = Model->AnimatedBoneAttributes.FindByPredicate([AttributeIdentifier](const FAnimatedBoneAttribute& Attribute)
		...
		if (AttributePtr)
		{
			...
			{
				UE_LOG(LogScript, Warning, TEXT("Attribute does not contain key for provided time"));
			}	
		}
		else
		{
...
bool UUMAnimDataController::DuplicateAttribute(const FAnimationAttributeIdentifier& AttributeIdentifier, const FAnimationAttributeIdentifier& NewAttributeIdentifier, bool bShouldTransact)
{
	ValidateModel();
	...
	if (AttributeIdentifier.IsValid() && NewAttributeIdentifier.IsValid())
	{
		if (AttributeIdentifier.GetType() == NewAttributeIdentifier.GetType())
		{
			...
			if (ExistingAttributePtr == nullptr)
			{
				...
				{
					UE_LOG(LogScript, Warning, TEXT("Could not find attribute with name %s and type %s for duplication"), ToCStr(AttributeIdentifier.GetName().ToString()), ToCStr(AttributeIdentifier.GetType()->GetName()));
				}
			}
			else
			{
				UE_LOG(LogScript, Warning, TEXT("Attribute with name %s already exists"), ToCStr(AttributeIdentifier.GetName().ToString()));
			}
		}
		else
		{
			UE_LOG(LogScript, Warning, TEXT("Attribute types do not match: %s (%s)"), ToCStr(AttributeIdentifier.GetType()->GetName()), ToCStr(AttributeIdentifier.GetType()->GetName()));
		}
	}
	else
	{
		UE_LOG(LogScript, Warning, TEXT("Invalid attribute identifier(s) provided: %s and/or %s"), ToCStr(AttributeIdentifier.GetName().ToString()), ToCStr(AttributeIdentifier.GetName().ToString()));
	}

	return false;
}

void UUMAnimDataController::UpdateWithSkeleton(USkeleton* TargetSkeleton, bool bShouldTransact)
{
#if WITH_EDITOR
#define LOCTEXT_NAMESPACE "AnimDataController"	
...
void UUMAnimDataController::PopulateWithExistingModel(TScriptInterface<IAnimationDataModel> InModel)
{
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
